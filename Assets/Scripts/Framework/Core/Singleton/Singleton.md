...
**이런 경우에 사용하세요**
- `Coroutine`, `Update` 등 Unity 기능이 필요하고 씬 전환 후에도 유지되어야 하는 경우
- SoundManager, ObjectPoolManager처럼 게임 전체 생명주기 동안 동작하는 시스템
- Inspector 설정 없이 코드만으로 완결되는 전역 매니저
...
씬에 오브젝트가 없으면 `Instance` 접근 시 자동으로 `GameObject`를 생성하고 `DontDestroyOnLoad`를 적용합니다.
씬에 직접 배치해도 동일하게 동작합니다.
...
| `PersistentMonoSingleton<T>` | 씬에 없으면 자동 생성되는 MonoBehaviour 싱글톤. `DontDestroyOnLoad`로 씬 전환 후에도 유지. |
...
```csharp
// 씬 유지 MonoBehaviour 싱글톤 (씬에 없으면 자동 생성, 씬 전환 후에도 유지)
...
SoundManager.Instance.PlayBGM(); // 어느 씬에서든 자동 생성 및 유지
```
...
| 버전 | 날짜 | 내용 |
|------|------|------|
| 1.2.0 | 2026-03-25 | PersistentMonoSingleton 자동 생성 추가 |
